...
from fastsurfer_nn.utils.checkpoint import get_paths_from_yaml
from fastsurfer_nn.utils.constants import REPO_ROOT
from fastsurfer_nn.utils.dataclasses import field, get_field
...
ALL_FLAGS = {
    ...
    "threads": __arg(
        ...
        type=int,
        help=f"Number of threads to use (default: {get_num_threads()})",
    ),
    "async_io": __arg(
        "--async_io",
        dest="async_io",
...
def add_plane_flags(
    ...
    files: Mapping[Plane, Path | str | None],
    ...
    files : Mapping[Plane, Path | str | None]
        ...
        Use None for optional planes (checkpoint can be omitted).
    defaults_path : Path, str
    ...
    def cast_type(__value: str) -> Path | None:
        # Support None values for both config and checkpoint (for optional planes)
        if not bool(__value) or __value.lower() == "none":
            return None
        return Path(__value)
    ...
    for plane, filepath in files.items():
        ...
        # If filepath is "default", load from defaults file
        # Otherwise, use the provided filepath
        if filepath is None:
            path = None
        elif str(filepath) == "default":
            path = defaults[plane]
