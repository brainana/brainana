...
- **A declared output-naming contract, and a validator for it.** brainana had no single rule for building an output name, so "which pipeline product is this" was written into whichever BIDS entity was nearest to hand. `src/nhp_mri_prep/utils/bids.py` now declares `ENTITY_ROLES` — every entity is *identity* (inherited from the raw data; brainana copies and never sets it), *frame*, *variant* or *product* — and `derive_output_name()` is the one constructor, raising rather than resolving the two ways a name loses information: overwriting an inherited identity entity, or emitting a key twice. `validate_output_name()` checks a published name, and `scripts/check_output_naming.py` checks a whole output tree (1417 names in the dev-test run) and exits non-zero on anything undeclared. `docs_temp/update_instruction/name_guideline.md` is the reference to read before adding an output
  - **The longitudinal stream is marked `space-base`, not `acq-base` / `acq-long`.** `acq` records how a scan was acquired and belongs to the raw data — the dev-test dataset carries a real `acq-test` — so writing a stream marker there destroyed a timepoint's own `acq-` label, and would have collapsed two acquisitions in one session onto a single filename, which `publishDir overwrite:false` makes a missing file rather than an error. `space` is what the distinction actually is: a base-seeded reconstruction lives in base space, already the documented reason the functional and fsnative-atlas streams keep the cross-sectional trees. Base derivatives now carry one space entity instead of `acq-base` plus a `space-T1w` that named a frame they were not in, and base atlases publish to `anat/atlas_space-base/`. Affects `session_longitudinal` only, which is unreleased — no published tree changes
  - **Nine QC figures carried two `desc-` entities** (`..._desc-preproc_desc-func2target_bold.png`). `parse_bids_entities` keeps only the last, so the round trip silently dropped `preproc` and the report saw a figure claiming less than it was. `QC_REGISTRATION_FUNC` named itself from the staged registered file rather than the run's naming template; it now goes through `derive_output_name`, and `create_bids_output_filename` de-duplicates *every* entity its suffix introduces rather than `space` alone
  - **`FUNC_APPLY_TRANSFORMS` declared one glob for two output slots**, so both received the whole sorted match list — and `..._bold.nii.gz` sorts before `..._boldref.nii.gz`, so the consumer that asked for the boldref was handed the 4-D BOLD and drew its func2target QC figure from it. The two slots now carry distinct globs
  - `BIDS_ENTITY_ORDER` gains the entities brainana emits but never listed — `atlas`, `from`/`to`/`mode`, `res`, `den`, `hemi`, `stat` — at the positions already published. They had fallen into the alphabetised `others` slot, which is emitted *before* `space` and `desc`, so a tSNR map rebuilt from its own entities came back as `..._stat-tsnr_desc-preproc_...`: a different file. Unknown raw entities now keep their original relative order instead of being alphabetised, so the two naming paths agree
  - Smaller fixes found by the same audit: the `desc-`→`desc-preproc` rewrite on publish was an unbounded `re.sub` that injected one `space-T1w` per `desc-` token, and stacked a second on an input already carrying a space; `_VALID_PREPROC_SUFFIXES` rejected `_space-T1w_desc-preproc_T2w_brain.nii.gz`; `create_bids_output_filename` turned a `_boldref` input into `sub-01_ses-001ref`; `isT1wFile` was a substring test that also passed for `_space-T1w_..._bold.nii.gz` and for any path with `T1w` in a directory name; `dataset_description.json` omitted the `BIDSVersion` the pipeline requires of its own inputs; `QC_BIAS_CORRECTION_FUNC` was dead; and the B₀ field-mapping dropdown links pointed at an anchor id the section never rendered
  - The scanner-space atlas rename replaces the frame token in place. Routing it through `replace_bids_space()` looked like a tightening, but that helper re-inserts at a BIDS anchor and an atlas name has neither a `_desc-` nor a trailing modality suffix, so it appended — `atlas-ARM1_sub-01_space-scanner.nii.gz`, which no longer matches the `atlas-<name>_space-*` shape every atlas consumer discovers by. Caught by diffing a real output tree against the previous run, and now pinned by a test
  - The `_brain` suffix tail, the subject-last atlas names and pass-through of unknown raw entities are **declared deviations**, recorded with the consumer that depends on each (brainana-viewer's data contract for the first two) rather than left looking accidental
  - The CI dependency workflow's PR path filter now includes `modules/**`, `workflows/**` and `main.nf`. Most output naming lives in the `.nf` process bodies, and a change confined to them previously ran no tests at all

- **`anat.synthesis_level: "session_longitudinal"` — within-subject longitudinal surface reconstruction.** A strict superset of `"session"`: identical anatomical selection, plus an unbiased within-subject base template per subject and a base-seeded reconstruction per session, so every timepoint inherits the base's mesh and vertex *i* is the same anatomical point throughout. That is what makes per-vertex differencing valid with no surface registration and no `?h.sphere.reg`, which brainana does not produce. The base is built with `mri_robust_template` alone — robust rigid registration plus median averaging, no atlas priors and no species assumptions, so it transfers to macaque data unchanged; `recon-all -base`/`-long` would run a human GCA volume stream and clobber the CNN segmentation the surfaces are built on. Requires `anat.surface_reconstruction.enabled` and at least two sessions with anatomy; one-session subjects are skipped rather than given a single-timepoint base, which would add an interpolation for no gain. Tunable under `anat.surface_reconstruction.longitudinal`: `iscale`, `subsample`, `max_cbv_dist`, `pial_blend_weight`, `time_column`
  - Published per subject: `fastsurfer/sub-<id>_base/` and `fastsurfer/sub-<id>_ses-<id>_long/`, base-space derivatives and atlases under `sub-<id>/anat/` marked `space-base`, and within-subject rates of change — `surf/?h.long.<measure>-{rate,avg,spc}.mgh`, `stats/?h.long.roi-rates.csv`, `stats/long.change-stats.json`, plus a `scripts/long.qdec.table.dat` for FreeSurfer's own longitudinal tools
  - The functional and fsnative-atlas streams deliberately keep seeing the cross-sectional trees: a `_long` tree's `orig.mgz` lives in base space, and `mri_vol2surf --regheader` assumes header agreement with the session's own volumes, so pointing them at longitudinal surfaces would misregister by exactly the timepoint-to-base transform
  - Documented in the processing details (§3.1), the outputs reference, and the anatomical-selection page; the configuration generator offers the new `synthesis_level` value and the four `longitudinal.*` knobs
  - `scripts/base_segmentation_agreement.json` records per-label Dice between the base's own segmentation and each timepoint's, mapped into base space. The CNN sees a different kind of volume on a robust average than it does cross-sectionally, and whether that domain shift matters is an empirical question; this makes it answerable from the run's own outputs. Deliberately not fused into a consensus — with two timepoints every disagreement is a tie, and breaking it would be a bias dressed up as a majority

- **Rates of change are fitted against real elapsed time when the dataset carries it.** `<bids_dir>/sub-XX/sub-XX_sessions.tsv` is read for an `age` or `acq_time` column (`acq_time` becomes days from the subject's first session; BIDS date-shifting preserves intervals, which is all a rate needs), or a column named by `anat.surface_reconstruction.longitudinal.time_column`. Without it the fit falls back to digits in the session label and then to scan order — for `ses-01/02/03` covering 2, 9 and 24 months that slope is wrong by an amount nothing in the outputs reveals. Resolution is all-or-nothing: a timepoint the table does not cover sends the whole subject back to the fallback, rather than mixing real ages and scan indices inside one regression. `long.change-stats.json` now records `time_source`, because a rate is only interpretable alongside the time it is per

- **Uncropped conformed anatomical (`desc-conformFullFOV`)** — the conform field of view is sized from the template, so it is a brain-sized box: a recording chamber, head-post, coil markers or the neck fall outside it and are cropped from every derivative, with no way to recover them. The conform step now also writes the same conform on a grid enlarged just enough to contain every voxel of the scanner-space input. Same transform, orientation and resolution — only the box is bigger, and the two grids differ by a whole number of voxels, so cropping the new image at that offset recovers the processed one (identically with the `sitk` backend; to floating-point precision with `flirt`, which redoes its own arithmetic from a re-expressed matrix). It is a leaf output: no downstream step reads it, and the processing pipeline is unchanged. Anatomical only; both rigid backends (`flirt`, `sitk`) are supported. If the enlarged grid cannot be sized, or would exceed a voxel cap, the run logs a warning and the output falls back to the target field of view rather than failing. The whole trio is declared an optional process output, so even an outright failure of the full-FOV helper leaves the output absent rather than failing the anatomical workflow, and conform QC still runs
- **The conform QC report now carries a pair of figures instead of one** — a new "full field of view" figure on the uncropped grid, with the processing field of view drawn as a lavender box, followed by the existing figure, which is that box enlarged. The captions state the relationship, so the crop is legible rather than invisible: hardware outside the box appears in the first figure and in no other output. The two are rendered as one block with no divider between them, since they are two views of a single conform step rather than two steps. Keeping the second figure matters — with the brain filling the frame its template contours are ~6x denser, so it stays the better view for judging alignment. The pair appears only when the grid was actually enlarged: when the input already fits the target field of view, or sizing fell back, the two figures would be pixel-for-pixel identical and only the existing one is rendered, captioned as before rather than pointing at a figure that is not there. The full-FOV figure is also rendered under its own error guard, so a failure on the enlarged grid — which is the memory-hungry one — costs only itself. The functional conform figure is unchanged

- **`func.confounds.fd_radius_mm` — the FD rotation radius is now configurable** — framewise displacement and rmsd convert rotations to millimetres on a sphere, and that radius was pinned at the macaque 27 mm with no way to reach it from configuration: `compute_confounds` accepted a `radius_mm` argument, but the step never passed one. Since this is `nhp_mri_prep`, any other primate silently got FD computed for a macaque head. The radius is now read from config and forwarded, and validated as a positive number. FD values are only comparable across runs computed at the same radius, so the radius actually used is recorded per column in the JSON sidecar
- **Outlier threshold reference lines on the confounds QC figure** — the FD and DVARS panels now draw a dashed line at the value above which a frame is flagged, so the red outlier bands are self-explanatory. Both criteria get a line, not just FD: a frame is flagged when *either* crosses, so an FD-only line would leave DVARS-driven bands looking unexplained. The line is drawn only when the threshold falls inside the panel's data range — on a clean run nothing approaches it, and forcing it into view would rescale the trace to a flat line; that case needs no explanation anyway, because a threshold nothing reaches produces no bands

- **Flagged frames are shaded on the confounds and motion QC figures** — the confounds TSV has always carried `non_steady_state_outlier##` and `motion_outlier##` indicator columns, but neither QC figure showed them, so a reader had to eyeball the FD panel and guess which spikes actually crossed a threshold, and could not tell at all where the unsteady-state block ended. Both figures now draw the flagged frames as vertical bands behind the traces: gray for non-steady-state (dummy) volumes, red for motion outliers. Contiguous frames merge into one band, and the bands are clamped to the shared frame axis, so the pixel alignment between the stacked motion and confounds figures is unchanged. Still indicators only — no volumes are removed from the BOLD data. The motion figure reads the confounds TSV as an *optional* input: when confounds are disabled, skipped or failed for a run, it is produced exactly as before, without bands
...
- **Brainana Lite publishes the outputs it was already computing** — the notebook now writes the uncropped `desc-conformFullFOV` conform and renders the paired conform QC figure, publishes the hemisphere mask, and records ingest-normalization provenance (`Input4DCollapsed`, `OrientationRecovered`, `QformSformReconciled`, `InputHeaderWarnings`) in the scanner-space sidecar. The conform step performs the full-FOV work unconditionally, so Lite was paying for the enlarged-grid resample on every run and discarding the result; the normalization repairs likewise happened but went unrecorded, which mattered most for `OrientationRecovered` — the flag carrying the left/right mirror caveat. A run that recovers an orientation now also prints that caveat
- **A `dataset_description.json` and a record of the effective configuration** — Lite output was not a valid BIDS-derivatives dataset (no `GeneratedBy`, version or template provenance at dataset level), and the configuration a run actually used was written nowhere. The description is written at the derivatives root and the merged config under `lite_reports/`
- **`tests/test_lite_notebook_api.py` — the Lite notebook is now covered by CI.** It calls brainana's step functions directly rather than going through Nextflow, and nothing checked that its imports, call signatures or config keys still resolved; the existing version guard only checked the ref string it pins. The new tests are static and import-only (no data, GPU or network) and additionally assert that the notebook's sparse checkout selects exactly the atlas files the runtime will later discover

### Changed

- **`_desc-conform_` intermediates are now matched by exact entity token, not substring** — the conform process excluded any filename containing `desc-conform` from publication and captured `*desc-conform*.nii.gz` as its output. Both now match `_desc-conform_`, so the new `_desc-conformFullFOV_` image publishes normally instead of being silently dropped and appended to the downstream channel
...
- **Lite fetches atlases the way the runtime selects them** — the sparse checkout required an exact `_space-{template}_res-05` match while `discover_atlases` matches by space and then picks the nearest resolution, so any atlas stored at another resolution was never downloaded and silently dropped from backprojection with no error and no warning. Worst for `TEMPLATE="D99"`, which discarded the D99 parcellation itself. Atlas-segmentation QC also now uses the pre-bias brain as its underlay, matching the anatomical workflow
- **Lite QC figures are published at `sub-XX/figures`, never under a `ses-` level**, matching where the pipeline puts them, and are named from the BIDS stem rather than a hand-assembled prefix, so `run-` and `acq-` entities survive on inputs used as-is
- **Config overrides in the Lite notebook go through a checked setter.** `load_config()` deliberately does not validate, so a mistyped key created a new entry nothing reads and the run continued on the default the user believed they had overridden. Two overrides that were already no-ops (`registration.fireants_allow_cpu`, which defaults true, and `general.anat_only`, read only by BIDS discovery) have been dropped

### Fixed

- **Confounds JSON sidecar reported the wrong rotation radius** — `RotationRadiusMM` was written from the module constant rather than the radius actually used, so an overridden radius produced a sidecar that disagreed with the framewise displacement values in the very TSV it describes. Now echoes the value passed in. Unchanged for the default 27 mm
- **`func.confounds` thresholds are validated** — only `enabled` was checked, so `fd_outlier_threshold_mm: loose` or a negative radius passed configuration validation and failed much later inside a Nextflow process, far from the cause. The three numeric keys are now checked as positive numbers up front (and `true` is rejected rather than read as `1.0`)

- **Motion QC step reported the wrong output path** — `qc_motion_correction` read `result["snapshot_file"]`, but `create_motion_correction_qc` returns the key `"motion_plot"`, so the lookup always fell through to its default. Harmless in practice (the default was the same path) but the recorded `qc_files` entry was never the value the snapshot function actually returned
...
- **Brainana Lite hung on Colab at "Ensuring SuiteSparse headers"** — the environment cell shelled out to `apt-get install libsuitesparse-dev` with no `apt-get update`, no non-interactive frontend, inherited stdin and no timeout, so it blocked indefinitely on the apt frontend lock or a debconf prompt, with `-qq` hiding all output. The step was also pointless: SuiteSparse is a build dependency of `scikit-sparse`, which lives in the `surf` extra and is never installed by `brainana[lite]`. Removed rather than hardened
- **Every Lite run re-cloned the repository and reinstalled from scratch** — the reuse check compared `rev-parse --abbrev-ref HEAD` against the configured ref, but checking out a tag leaves a detached HEAD where that command returns the literal string `HEAD`, so the check could never succeed for the pinned default. The notebook deleted and re-cloned on every Run All while printing "(Install is skipped on the second pass.)" The check now resolves the ref to a commit, so it works for tags, branches and SHAs alike
- **`RUN_DEMO=True` could not be run twice** — the example volume is downloaded into `WORKING_DIR`, which is exactly where the input check looks, so a second Run All aborted on the file the first run had created and told the user to delete it. The check now exempts the known demo filename, which also makes the existing re-use branch reachable
- **The Lite notebook cloned the wrong repository** — `BRAINANA_URL` still named the pre-migration `xingyu-liu/brainana` while the Colab badge had moved to `brainana/brainana`, so users opened one repository's notebook and ran another repository's code. Documentation links were split the same way and are now consistent
- **Network calls in the Lite environment cell are bounded** — the clone, the uv bootstrap and the demo download had no timeout, and the demo download set no socket timeout either. A notebook has no stdin, so a credential prompt or a stalled connection hung the cell forever with no output. A failed download no longer leaves a partial file that the input check would read as user data
- **A single failed QC figure no longer destroys a completed Lite run** — the notebook called the snapshot functions directly, without the `errorStrategy 'ignore'` the pipeline gives every QC process, so a plotting error aborted the notebook after all the expensive compute had succeeded. Figures that fail are now reported and skipped
- **Removed dead code from the Lite notebook** — a patch that rewrote a QC import in the cloned source tree, whose guard has existed in that source since before 2.1.0, so it never fired; and an unused import. The optional FireANTs `fused_ops` build no longer logs a green check mark when the build failed
